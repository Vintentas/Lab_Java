...
public class Count {
  ...
  static private int countTrainer=1; //счетчик тренера
  static private int countSportsman=1; // счетчик спортсмена
  ...
  //        возвращает либо кол-во выполненных заданий, либо 0;
    static public synchronized int counterExercise (int a, String name){
        ...
        if(name=="trainer" & a<countTrainer & countTrainer<=countSportsman){    
            System.out.println("Я тренер. Я даю задание " + countTrainer + ".");  
            return  countTrainer++;
        }
        if(name=="sportsman" & a<countSportsman & countSportsman<countTrainer){  
            System.out.println("Я спортсмен. Я выполняю задание " + countSportsman + "."); 
